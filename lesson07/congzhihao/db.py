...
def Expert_Xls(cur):
    ...
    if num == 0:
        ...
        wbk.save("user.xls")
        print("导出成功，文件位于{}".format(os.getcwd()+"/user.xls"))
        logging.info("excel文件导出成功！")

def Expert_Html(cur):
    logging.debug("导出html文件操作")
    num = cur.execute("select * from users;")
    if num == 0:
        print("无用户数据，导出失败")
        logging.warning("无用户数据，导出失败")
        ...
        tem_env = Environment(loader=template_loader)
        tem_file = "user_print_template.html"
        tem = tem_env.get_template(tem_file)
        data =dict()
        data["userinfo"] = user_info
        html_code = tem.render(data)
        with open("user.html", 'w') as f:
            f.write(html_code)
        print("导出成功，文件地址：{}".format(os.getcwd()+"/user.html"))
