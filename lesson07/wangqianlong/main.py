...
#         # stat = useradmin.http_auth(username, password)
...
#                         elif int(text) == 4:
#                             useradmin.user_query(CONFIGFILE2, PAGE_SIZE)
#                         elif int(text) == 5:
#                             '此部分实现导出到csv文件或xls部分' \
#                             '1.实现从数据库读出所有数据' \
#                             '2.将数据存到xls'
...
#                                 useradmin.page_print(userinfo, PAGE_SIZE)  # 分页显示
...
def admin_main():
    ...
    while True:
        ...
        if stat.ok:
        # if stat == 200:
            if not useradmin.is_unlock(CONFIGFILE, 'DB', 'LOCKFILE', LOCK_DURATION):
                print('用户锁定！')
            else:
                print(LOGIN_SUCCESS)
                '''
                ...
                DB.Create_table(DATABASE, CREATE_TABLE)
                '''
                mapFunc = {1: useradmin.user_add, 2: useradmin.user_delete, 3: useradmin.user_update,
                           4: useradmin.user_query, 5: useradmin.user_list, 6: useradmin.user_html}
                while True:
                    try:
                        text = int(input('请输入对应操作数字(1-7):'))
                        if text < 7 and text > 0:
                            mapFunc[text](CONFIGFILE)
                        elif int(text) == 7:
                            print('退出系统.....')
                            break
                        else:
            ...
            if login_count == MAX_LOGINTIMES:
                print('\033[0;31m认证失败3次，用户锁定！\033[0m')
                # logging.warning("失败3次，用户锁定！")
                useradmin.lock_user(CONFIGFILE, 'DB', 'LOCKFILE')
                break
            else:
                print('\033[0;31m认证失败，请重新认证!\033[0m')
                # logging.warning("登录失败，请重新输入!")
