...
def http_auth(username, password):
    # headers = {'Authorization': 'token ' + TOKEN}
    try:
        req = requests.get('http://api.github.com/user', auth=(username, password))
        return req
...
# 实现翻页操作
def page_updown(users, page_count, PAGE_SIZE):
    count1 = 1
    while count1 <= page_count:
        ...
        x, flag = utils.PrintTable(userinfo_list_page)
        if flag:
            print(x)
        nextorback = input('\033[0;33m请输入：\033[0m')
        if nextorback == 'down':
            count1 += 1
        ...
        elif nextorback.isdigit():
            if int(nextorback) <= page_count and int(nextorback) > 0:
                count1 = int(nextorback)
...
# 分页显示所有用户；
def page_print(users, PAGE_SIZE):
    if len(users) == 0:
        ...
        if page_count == 1:
            ...
            分页导出功能介绍：
                    ...
                    2.输入页码直接显示
                    3.输入exit退出分页功能
                    4.退出分页输入yes可导出用户信息到xls
            ---------------------------------
                  \033[0m '''
            print(msg)
            page_updown(users, page_count, PAGE_SIZE)


def user_html(CONFIGFILE2):
    list_sql = ''' select * from {};'''.format(USERINFO)
    users_find, ok = DB.Select(CONFIGFILE2, list_sql)
    if ok:
        '''users信息转为字典形式。
        content = {'userinfo': [
            {'id': 1, 'username': 'monkey1', 'age': 20, 'tel': '132xx', 'address': 'beijing',
             'create_time': '2018-09-16 12:49:14.486554', 'update_time': '2018-09-16 12:49:14.486554'},
            {'id': 2, 'username': 'monkey2', 'age': 20, 'tel': '132xx', 'address': 'beijing',
             'create_time': '2018-09-16 12:49:14.486554', 'update_time': '2018-09-16 12:49:14.486554'},
        ]
        }'''
        userinfo = []
        ...
        path_table = os.path.join(path1, 'table')
        print(path_table)
        template_loader = jinja2.FileSystemLoader(searchpath=path_table)

        template_env = jinja2.Environment(loader=template_loader)
        template = template_env.get_template("usersManage.html")
        html_str = template.render(content)
        print(html_str)

        path_html = os.path.join(path_table, 'users.html')
