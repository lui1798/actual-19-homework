...
def query():
    try:
        ...
        data = conn_db(sql)
        print('|{:^5}|{:^10}|{:>3}|{:^18}|{:>20}|{:^20}|{:^20}'.format("id", "name", "age", "tel", "address", "create_time", "update_time"))
        print('{:*^105}'.format('*'))
        for i in data:
            i = list(i)
            i[5] = str(i[5])
            i[6] = str(i[6])
            print('|{:^5}|{:>10}|{:>3}|{:^18}|{:>20}|{:>20}|{:>20}'.format(i[0], i[1], i[2], i[3], i[4], i[5], i[6]))
...
def config():
    try:
        conf_keys = input('要查看关键字(LOG|DB|CSV):')
