...
def add_user(userList):
    ...
    if len(userInfo) != 4:
        ...
        logging.debug('your information added successfully!')
...
def delete_user(userList):
    if len(userList) != 0:
        ...
        if delId in [x['userId'] for x in userList]:
            ...
            logging.debug('your information deleted successfully!')
        else:
            print("invalid input")
            logging.warning('invalid input')
    else:
        print("the useList is empty,please add information first")
        logging.warning('the useList is empty')
    ...
    if len(updateInfo) == 3:
        ...
        if uid in [x['userId'] for x in userList]:
            ...
            if elementType in ['name', 'age', 'telNumber', 'address']:
                ...
                logging.warning('input is invalid！')
        else:
            print("the id you input is not exist.")
            logging.warning('the id you input is not exist.')
...
def user_main():
    if check_lock():
        ...
        logging.warning('user is locked, please try again later！')
        exit()
    while True:
