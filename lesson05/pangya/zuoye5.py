...
PAGE_SIZE = 6
# 导入模块
import json, datetime, os, math, logging
    ...
    while count > 0:
        ...
        if username == LOGIN_INFO[0]:
            if password == LOGIN_INFO[1]:
                ...
                logging.warning("你输入的密码错误!")
                count -= 1
        else:
            logging.warning("你输入的用户名错误!")
    if count == 0:
        logging.error("你的账号已被锁")
...
def add_users(userinfo):
    ...
    if len(user_list) != USER_INFO_NUM:
        ...
        has_error = False
        if not tel.isdigit():
            logging.warning("你输入的电话号码不是数字")
            has_error = True
        elif not age.isdigit():
            logging.warning("你输入的年龄不是数字")
            has_error = True
        if not has_error:
...
def update_users(userinfo):
    ...
    if is_exists:
        ...
        if len(user) != USER_INFO_NUM:
            logging.warning("输入数据不正确")
        else:
            name, age, tel, address = user
            has_error = False
            if not age.isdigit():
                has_error = True
                logging.warning("输入的年龄不正确,请输入正确的年龄")
            if not tel.isdigit():
                has_error = True
                logging.warning("输入的电话不正确，请输入正确的电话")
            if not has_error:
                for user in userinfo:
                    if uid == user.get("id"):
                        user["name"] = name
