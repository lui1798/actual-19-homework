...
#标准化输出
def echo():
    print('|{:^5}|{:>10}|{:>15}|{:^5}|{:>20}|'.format("id", "name", "tel", "age", "address"))
    print('{:*^60}'.format('*'))
...
def list():
    ...
    while True:
        ...
        for i in user_list[begin:end]:
            print('|{:^5}|{:>10}|{:>15}|{:^5}|{:>20}|'.format(i["id"], i["name"], i["tel"], i["age"], i["address"]))
        page = math.ceil(len(user_list) / increment)
        print('一共{}页'.format(page).rjust(60))
...
def add():       # 添加用户
    try:
        ...
        writefile(user_dict)
        print('用户{}添加成功'.format(user_list[1]))
        logging.debug('用户 {} 添加成功, 用户信息为 {}'.format(user_list[1],userinfor))
    except Exception as e:
        print('用户添加失败')
        logging.warning('用户 {} 添加失败')
...
def delete():    # 删除用户
    ...
    if flag:
        ...
        if not flag:
            ...
            logging.debug('用户"{}" 已成功删除.'.format(userinfo))
        else:
            print('无此用户')
            logging.warning('输入用户{} ,无此用户信息.'.format(userinfo))
    ...
    for i in user_list:
        if userinfo == i['name']:
            ...
            if update_befor == 'name':
                ...
                print('输入有误,请重新输入.')
                logging.warning('UPDATE:输入有误,请重新输入 ')
                flag = False
            print('用户"{}" 已成功更新.'.format(userinfo))
            logging.debug('UPDATE: 用户"{}" 已成功更新. 跟新了{}.'.format(userinfo,update_befor))
            flag = False
                ...
                with open('lock.txt','r') as f:
                    ...
                    if time:
                        ...
                        if time_tmp_1 >= datetime.datetime.now():
                            ...
                            logging.debug('User is locked !!!')
                            break
                    else:
                        print('管理员登陆成功')
                        logging.debug('User {} ,Password xxxx access successful'.format(username))
                while True:
                    info = '''
* * * * * * * * * * * * * * * *
*      1:查看全部用户          *
