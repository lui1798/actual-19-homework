...
while True:
    ...
    if ret[0] == -1:
        ...
        logging.war('##################用户名或密码错误,一天只有三次机会,您没有机会了!##################')

        exit(0)
    elif ret[0] == 0:
        print('用户名或密码错误,还剩{}次机会!'.format(ret[1]))
        logging.war('用户名或密码错误,还剩{}次机会!'.format(ret[1]))
        continue
