#导入模块
import datetime, json, math, logging

#日志logging
logging.basicConfig(
    filename = "admin_sys_v4.log",
    filemode = 'a',
...
def users_delete(users):
    ...
    if is_del_error:
        ...
        logging.debug(f"delete操作：{user}")
    else:
        print("输入错误")
        logging.warning("delete操作：输入错误")
    return users
...
def users_modify(users):
    ...
    if is_modify_exists:
        ...
        if is_error:
            print("用户信息输入错误")
            logging.warning("modify操作：用户信息输入错误")
        #录入正确
        if not is_error:
            user_up["name"] = name
            user_up["age"] = age
            ...
            logging.debug(f"modify操作：{user_up} ")
    #id输入错误
    else:
        print("id输入错误")
        logging.warning("modify操作：id输入错误")
    return users
...
def users_list(users):
    ...
    if users_find_count == 0:
        print("查询结果：无")
        logging.warning("list操作：查询失败 ")
    #字典格式化输出
    elif users_find_count <= PAGE_SIZE:
        logging.debug("list操作：查询成功 ")
...
def option(users):
    while True:
        ...
        if op == '1':
            logging.debug("进入add操作")
            users_add(users)
        elif op == '2':
            logging.debug("进入delete操作")
            users_delete(users)
        elif op == '3':
            logging.debug("进入modify操作")
            users_modify(users)
        elif op == '4':
            logging.debug("进入list操作")
            users_list(users)
        elif op == '6':
            logging.debug("进入save操作")
            users_save(users)
        elif op == 'q':
            logging.debug("进入exit操作")
            users_exit(users)
        elif op == '5':
             list_all(users)
             logging.debug("进入list_all操作")
        else:
            logging.warning("操作选择错误")
            print("输入错误")
