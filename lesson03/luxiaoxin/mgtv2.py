...
#系统登录
while True:
# 读取用户登录失败时间文件,文件在脚本同一目录下如果没有则系统新建login.txt存储数据
    with open('login.txt', 'a+t') as fhandler:
        fhandler.seek(0)
        login = fhandler.read()
        if login != '':
            login_time = datetime.strptime(json.loads(login), "%Y-%m-%d %H:%M:%S")
    ...
    if login_agree == 0:
        print('\033[31mLogin failed more than 3 times in 1 days. Please try again tomorrow!\033[0m')
        break

    if login_num >= 3:
        with open('login.txt', 'wt') as fhandler:
            time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            membuf = json.dumps(time)
            fhandler.write(membuf)
        break
    else:
                ...
                if op == 'list':
                    ...
                    while True:
                        if page_end == 0:
                            ...
                            for i in userlist[row_num : row_num + 5]:
                                print(tpl_users.format(str(i['ID']), i['NAME'], i['AGE'], i['TEL'],i['ADDRESS']))
                    ...
                    if len(userdata) == 4 and userdata[3] != '':
                        if len(userlist) == 0:
                            ...
                            uid = str(max([int(i['ID']) for i in userlist]) + 1)

                        userdata.insert(0, uid)

                        for i in range(len(colums_titile)):
                            userdict[colums_titile[i]] = userdata[i]

                        userlist.append(userdict)
                        #打印添加的用户信息
                        print('\033[32mAdd user success!~\033[0m')
                        print('{0}\n{1}\n{2}\n{3}\n{4}'.format(split_titile, titile, split_titile,
                                                               tpl_users.format(str(userdict['ID']), userdict['NAME'],
                                                                                userdict['AGE'], userdict['TEL'], userdict['ADDRESS']),
                                                               split_titile))
                        #存储用户信息到文件
                    ...
                    if uid not in [userlist[i]['ID'] for i in range(len(userlist))]:
                        ...
                        if len(userdata) == 4 and userdata[3] != '':
                            for i in userlist:
                                if uid == i['ID']:
                                    i['NAME'],i['AGE'],i['TEL'],i['ADDRESS'] = userdata[0],userdata[1],userdata[2],userdata[3]
                                    # 打印修改的用户信息
                                    print('\033[32mUpdate user data success!~\033[0m')
                                    print('{0}\n{1}\n{2}\n{3}\n{4}'.format(split_titile, titile, split_titile,
                                                                           tpl_users.format(str(i['ID']), i['NAME'], i['AGE'], i['TEL'],
                                                                                            i['ADDRESS']), split_titile))
                                    # 存储用户信息到文件
                                    with open('userdata.txt', 'wt') as fhandler:
                                        membuf = json.dumps(userlist)
                                        fhandler.write(membuf)
                    ...
                    user = user.strip()
                    if ((user not in [userlist[i]['ID'] for i in range(len(userlist))]) and (user not in [userlist[i]['NAME'] for i in range(len(userlist))])):
                        print('\033[31mUser does not exist!~\033[0m')
                    else:
                        for i in userlist:
                            if user == i['NAME'] or user == i['ID']:
                                print('{0}\n{1}\n{2}\n{3}\n{4}'.format(split_titile, titile, split_titile,
                                                                       tpl_users.format(str(i['ID']), i['NAME'], i['AGE'], i['TEL'],
                                                                                            i['ADDRESS']), split_titile))
