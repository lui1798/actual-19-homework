...
TABLE_TEMPLATE = '{num:>5d}|{name:>8s}|{age:>5d}|{tel:>15s}|{city:>10s}'
TABLE_LINE = 47
...
def add_user(users):
    ...
    if len(adduser.split()) == 4:
        ...
        if not age.isdigit():
            print('年龄输入有误')
        if tel.isdigit() == False or len(tel) != 11:
            print('电话输入有误')
        else:
...
def list_user(users):
    ...
    for user in users:
        ...
        or user.get['age'].find() != -1 \
        or user.get['tel'].find() != -1 \
        or user.get['city'].find() != -1:
